...
import styles from "./PeppolInvoiceView.module.scss";
...
function PeppolInvoiceView({ invoice, intl }) {
    const { billDetail, invoiceNo } = invoice;
    const sbd = XMLUtils.parse(billDetail);
    const bisDoc = XMLUtils.node(sbd, "/sbdh:StandardBusinessDocument/ubl-invoice-2:Invoice");
            ...
            <div className={styles.pageContainer}>
                <div className={styles.billDetailsContainer}>
                    <BISDetailsCard
                        invoice={invoice}
                        bisDoc={bisDoc}
                        bisTypeCode={XMLUtils.text(bisDoc, "./cbc:InvoiceTypeCode")}
                        intl={intl}
                        headingIntlKey="invoice.view.documentDetails.heading"
                        typeLabelIntlKey={"peppol.invoice.view.documentDetails.invoiceType.label"}
                    />
                    <BISLineItemsCard
                        lines={XMLUtils.nodes(bisDoc, "./cac:InvoiceLine")}
                        intl={intl}
                    />
                    <PartyCard
                ...
                <div className={styles.pdfViewContainer}>
                    {<PdfPreviewCard invoice={invoice} intl={intl} title="invoice.view.preview.heading"
...
export default withRouter(injectIntl(PeppolInvoiceView));
